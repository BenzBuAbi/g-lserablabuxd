...
exports.run = async (client, message, args) => {
  ...
  if(!message.member.roles.get("634765597271654411") && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send("Bu Komutu Kullanmaya Yetkin Yok!!").then(m => m.delete(5000));
  ...
  var user = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[0]));
    ...
    var muterole = message.guild.roles.get("634842083966058527")
  ...
  .setTimestamp()  
    ...
    let sChannel = message.guild.channels.get("751471434534224003")
    if(!sChannel) return
    sChannel.send(embed)
  },1000)
    },  ms(muteTime));
...
`)
  ...
  .setTimestamp()  
    let sChannel = message.guild.channels.get("762419595575164948")
    if(!sChannel) return
    sChannel.send(embed)
